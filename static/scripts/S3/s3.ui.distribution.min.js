...
a=="return"&&d.text(c.lossLabel);return e},_renderItemRow:function(a,c){var d=this.options;let e=a.quantity-0;isNaN(e)&&(e=a.max-0||0);let g=b('<tr class="dist-item">').data({mode:a.mode||(c=="return"?"RET":null),itemID:a.id,packID:a.pack_id,quantity:e,max:a.max,itemQ:e,lossQ:0});var h=b('<input type="checkbox" class="select-item">');b("<td>").append(h).appendTo(g);h=b("<div>").text(a.name);h=b("<td>").append(h).appendTo(g);a.mode=="LOA"&&b('<div class="dist-mode">').text(d.loanLabel).appendTo(h);
d=b('<input type="number" min="0" class="item-q">').val(e).prop("disabled",!0);b("<td>").append(d).appendTo(g);c=="return"?(c=b('<input type="number" min="0" class="loss-q" value="0">').prop("disabled",!0),b("<td>").append(c).appendTo(g)):b("<td>").appendTo(g);a=b("<div>").text(a.pack);b("<td>").append(a).appendTo(g);return g},_renderMessageRow:function(a){return b("<tr>").append(b('<td class="blocked-msg" colspan="5">').text(a))},_updateActionDetails:function(){var a=!1,c=[],d=[],e={d:c,r:d};b(".dist-item.selected",
this.detailsContainer).each(function(){let g=b(this),h=g.data();if(b(".invalidinput",g).length)a=!0;else if(h.itemQ||h.lossQ)h.mode=="RET"?d.push([h.itemID,h.packID,"RET",h.itemQ,h.lossQ]):c.push([h.itemID,h.packID,h.mode,h.itemQ])});a||!c.length&&!d.length?this.actionDetails.val(""):this.actionDetails.val(JSON.stringify(e))},_checkActionable:function(){return!!this.actionDetails.val()},_selectItem:function(a){a=b(a);const c=a.closest("tr.dist-item"),d=c.data("quantity");b(".error",c).remove();a.prop("checked")?
...
c=b("a.event-type-select",this.distributionSetSelect).length,d=this.distributionSet.val();var e=this.options;d?a.removeClass("challenge"):(e=c>0?e.selectDistributionSetLabel:e.noDistributionSetsLabel,b(".event-type-name",a).text(e));c==0?a.addClass("empty").addClass("disabled"):c==1&&d?a.removeClass("empty").addClass("disabled"):(a.removeClass("empty").removeClass("disabled"),d||a.addClass("challenge"))},_populateDistributionSets:function(a){let c=this.distributionSetSelect.empty(),d=a.sets;(a=a.default)||
d.length!=1||(a=d[0]);d.forEach(function(e){let g=e[0];e=e[1];b('<a class="secondary button event-type-select">').text(e).appendTo(c).data({id:g,name:e})});a?this._setDistributionSet(a[0],a[1]):this._clearDistributionSet()},_selectOrg:function(a){let c=a.data("id");a=a.data("name");let d=this.orgHeader,e=this.orgSelect;this._clearForm();d.data("selected",c).addClass("selected");b("h4.org-name",d).text(a);let g=b('<div class="inline-throbber">'),h=this.distributionSetHeader.addClass("disabled"),l=
b(".event-type-name",h).hide().after(g),k=this;b.ajaxS3({url:this.options.ajaxURL+"?org="+c,type:"GET",dataType:"json",contentType:"application/json; charset=utf-8",success:function(f){k._populateDistributionSets(f);g.remove();l.show();f.sets.length>1&&h.removeClass("disabled")},error:function(){g.remove()}});e.slideUp("fast")},_selectDistributionSet:function(a){let c=a.data("id");a=a.data("name");this._setDistributionSet(c,a);this.distributionSetSelect.slideUp("fast")},_bindEvents:function(){const a=
