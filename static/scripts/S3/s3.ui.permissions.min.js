...
rm_SystemTables:"System Tables",rm_Table:"Table",rm_UnrestrictedTables:"Unrestricted Tables"},w=function(a){var b,c=a[3];c?c!==!0&&(b=c):a[2]!==!0&&(b=a[1]+"/"+(a[2]||"*"));return b},y=0;d.widget("s3.permissionEdit",{options:{fRules:!1,tRules:!1,useRealms:!1,permissions:[{l:"CREATE",b:1,o:!1},{l:"READ",b:2,o:!0},{l:"UPDATE",b:4,o:!0},{l:"DELETE",b:8,o:!0}],modules:{},models:{},defaultPermissions:{},icons:{expanded:"fa fa-caret-down",collapsed:"fa fa-caret-right"}},_create:function(){this.id=y;y+=
...
Object.keys(b.modules).sort().forEach(function(g){b.modules[g][1]&&f._ruleTable("p",g,k[g]).appendTo(h)});k._other.length&&this._ruleTable("p","_other",k._other).appendTo(h);b.tRules&&(k=c.tables,h=d(".rm-table-rules",a),this._renderExpandCollapseAll().appendTo(h),Object.keys(b.models).sort().forEach(function(g){g!="_system"&&f._ruleTable("t",g,k[g]).appendTo(h)}),this._ruleTable("t","_system",k._system).appendTo(h),k._other.length&&this._ruleTable("t","_other",k._other).appendTo(h));e.tabs().removeClass("hide").show();
this._bindEvents()},_deserialize:function(){try{this.rules=JSON.parse(this.input.val())}catch(a){this.rules=[],this._serialize()}},_serialize:function(){var a=this.rules.filter(function(b){return b[0]!==null&&(b[0]!==0||!b[7])});this.input.val(JSON.stringify(a)).trigger("change")},_ruleSets:function(){var a={_other:[]},b={_other:[]},c=this.options;this.rules.forEach(function(e){var f;if(f=e[3]){var k=b;var h=c.models;f=f.split("_")[0]}else{k=a;h=c.modules;f=e[1];var g=e[2];f=f!="default"||g!="table"&&
g!="tables"?f:"default/dt"}h.hasOwnProperty(f)?k[f]===u&&(k[f]=[]):f="_other";k[f].push(e)});return{pages:a,tables:b}},_renderExpandCollapseAll:function(){var a=d('<span class="action-lnk rm-expand-all">'),b=d('<span class="action-lnk rm-collapse-all">');a.text(l.rm_ExpandAll);b.text(l.rm_CollapseAll);return d('<div class="rm-toggle-all">').append(a).append(b)},_ruleTable:function(a,b,c){c||=[];var e=d('<table class="rm-module-rules">').data({module:b});e.append(this._ruleTableHeader(b)).append(this._ruleTableBody(a,
...
a=l.rm_DynamicTables;break;default:f=a=="default/dt"?"DEFAULT":a.toLocaleUpperCase(),a=this.options.modules.hasOwnProperty(a)?this.options.modules[a][0]:""}d('<div class="rm-module-prefix">').text(f).appendTo(c);d('<div class="rm-module-title">').text(a).appendTo(c);d('<div class="rm-module-numrules">').appendTo(c);d("<tr>").append(c).appendTo(b);return b},_ruleTableBody:function(a,b,c){var e=this.options,f=d("<tbody>").hide(),k=!0,h=c.map(w);if(a=="t"){if(b=e.models[b])for(var g in b)b[g]&&h.indexOf(g)==
-1&&c.push([null,null,null,g,null,null,null,!1]);h=l.rm_Table;c.length===0&&(k=!1,f.append(this._defaultRule(!1)))}else{g=b=="default/dt";var n;for(n in e.defaultPermissions)if((a=g?n=="default/tables"||n=="default/table":n.slice(0,b.length+1)==b+"/")&&h.indexOf(n)==-1){a=n.split("/");var r=a[1];if(r=="*")r=null;else if(!e.fRules)continue;c.push([null,a[0],r,null,null,null,null,!1])}h=l.rm_Page;c.length===0&&(k=!1,f.append(this._defaultRule(!0)))}b=d('<tr class="rm-rule-labels">').hide().appendTo(f);
b.append("<th>"+h+"</th>").append("<th>"+l.rm_AllRecords+"</th>").append("<th>"+l.rm_OwnedRecords+"</th>");e.useRealms&&d("<th>").text(l.rm_Scope).appendTo(b);b.append("<th></th>");if(k){b.show();c=c.sort(function(m,p){m=m.slice(1,4).join(" ");p=p.slice(1,4).join(" ");return m===p?0:m>p&&1||-1});var q=this;c.forEach(function(m){q._ruleTableRow(m).appendTo(f)})}return f},_ruleTableFooter:function(a,b,c){var e=d("<tfoot>").hide();if(b!="_other"){b=d('<tr class="rm-module-add">');var f=d("<td>").attr("colspan",
this.tableWidth);d('<span class="action-lnk rm-add-rule">').text(l.rm_AddRule).appendTo(f);a!="t"&&!this.options.fRules&&c.length&&b.hide();b.append(f).appendTo(e)}return e},_ruleTableRow:function(a){var b=d('<tr class="rm-rule">').data({rule:a}),c=this.options,e=w(a);if(e)d('<td class="rm-rule-target">').text(e).appendTo(b);else{var f=this._targetSelector(a);d('<td class="rm-rule-target">').append(f).appendTo(b)}e&&a[0]===null?(b.addClass("rm-default-permissions"),a=this._defaultPermissions(e),a.oACL?
...
r=0,q=0;Object.keys(h).sort().forEach(function(m){h[m]?(r+=1,g.append(f(m))):(q+=1,n.append(f(m)))});a=d("<select>").append(k());r&&a.append(g);q&&a.append(n)}else e=="_other"?(a=d('<input type="text" size="20">'),a.data("pattern",/^(\w+)(\/(\*|[\w]+))?$/)):e=="default/dt"?(a=d("<select>"),a.append(k()).append(f("default/table")).append(f("default/tables"))):(c.append("<span>"+e+" / </span>"),a=d('<input type="text" size="10">'),a.data("pattern",/^(\w+|\*)$/));a.addClass("rm-target-select").appendTo(c);
