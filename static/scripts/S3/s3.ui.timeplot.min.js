...
f(b).append(a)}},_renderChart:function(a){var b=this.chart;if(b){var d=this.options,e=this.currentChart,c=null,g=null;a?(c=a.type,g=a.axis):(e&&(c=e.type,g=e.axis),c&&g||(c=d.defaultChartType,g=d.defaultChartAxis));e.type==c&&e.axis==g||this._removeChart();a=this.element;d=a.find(".tp-empty");e=this.data;e===null&&(e=JSON.parse(this.input.val()));e||={e:!0};this.data=e;if(e.e)this._removeChart(),a.find(".tp-empty").show();else switch(d.hide(),b.show(),g){case "totals":switch(c){case "barchart":this._renderMultiBarChart(b,
e.f,e.p);break;default:this._renderLineChart(b,e.f,e.p)}}}},_removeChart:function(){var a=this.chart;a&&(a.empty().hide(),a=this.currentChart,a.container=null,a.chart=null,a.type=null,a.axis=null,f(window).off("resize.tp"))},_renderMultiBarChart:function(a,b,d){var e=v(b,d);d=this.currentChart;var c=this.options;if(d.chart){var g=d.container;var k=d.chart;k.showControls(b.length>1);g.datum(e).transition().duration(50).call(k)}else f(a).closest(".tp-chart-contents").show(),f(a).css({height:"360px"}),
g=d3.select(f(a).get(0)).append("svg").attr("class","nv"),k=nv.models.multiBarChart().x(function(h){return h.start}).y(function(h){return h.value}).staggerLabels(!0).showControls(b.length>1).controlLabels({grouped:c.labelGrouped,stacked:c.labelStacked}).forceY([0,1]),k.tooltip.enabled(!1),k.yAxis.tickFormat(this.options.numberFormatter),k.xAxis.tickFormat(function(h){return(new Date(h)).toLocaleDateString()}),nv.addGraph(function(){g.datum(e).transition().duration(50).call(k);f(window).off("resize.tp").on("resize.tp",
...
1]),g.yAxis.tickFormat(this.options.numberFormatter),g.xAxis.tickFormat(function(k){return(new Date(k)).toLocaleDateString()}),nv.addGraph(function(){c.datum(e).transition().duration(50).call(g);f(window).off("resize.tp").on("resize.tp",function(k){g.update(k)});return g}),b.container=c,b.chart=g,b.type="linechart",b.axis="totals"},reload:function(a,b,d){d=typeof d!="undefined"?d:!0;typeof b=="undefined"&&(b=this._getFilters());var e=this,c=!1;f(this.element).find(".tp-throbber").show();if(a||b)c=
this._updateAjaxURL(a,b);c||d?(a=this.options.ajaxURL,b=f.ajaxS3,f.searchS3!==z&&(b=f.searchS3),b({url:a,type:"GET",dataType:"json",success:function(g){e.input.val(JSON.stringify(g));e.data=g;e.refresh()},error:function(g,k,h){console.log(h=="UNAUTHORIZED"?i18n.gis_requires_login:g.responseText)}})):e.refresh()},_getOptions:function(){var a="#"+f(this.element).attr("id"),b=f(a+"-time").val(),d=null,e=null,c=f(a+"-slots").val();b!="custom"&&(b=b.split("|"),b.length==3&&(d=b[0],e=b[1],c&&c!="auto"||
(c=b[2])));return{fact:f(a+"-fact").val(),timestamp:f(a+"-timestamp").val(),start:d,end:e,slots:c}},_getFilters:function(){var a=f("#"+this.widget_id+"-filters");try{return a.length?S3.search.getCurrentFilters(a.first()):null}catch(b){return null}},_updateAjaxURL:function(a,b){var d=this.options.ajaxURL;if(!d)return!1;var e=d.split("?");if(e.length>1){var c=e[1];var g=c.split("&")}else c="",g=[];var k=[];d=!1;if(a)for(var h in a)c=a[h],c=h+"="+c,d||f.inArray(c,g)!=-1||(d=!0),k.push(c);h={};var p=
{},n;if(b){var q=function(x){var t,u,y=[];["contains","anyof","belongs","eq"].forEach(function(r){t=new RegExp("__"+r+"$","g");x.match(t)?u=t:y.push(r)});u&&y.forEach(function(r){p[x.replace(u,"__"+r)]=!0})};c=0;for(n=b.length;c<n;c++){var m=b[c];var l=m[0];m=m[1];q(l);m===null?h[l]||(p[l]=!0):(p[l]&&(p[l]=!1),m=l+"="+encodeURIComponent(m),h[l]?h[l].push(m):h[l]=[m])}}c=0;for(n=g.length;c<n;c++)m=g[c].split("="),m.length>1&&(l=decodeURIComponent(m[0]),m=decodeURIComponent(m[1]),p[l]?d=!0:h[l]?d||
f.inArray(l+"="+m,h[l])!=-1||(d=!0):a&&a.hasOwnProperty(l)||k.push(g[c]));for(l in h)for(c=0,n=h[l].length;c<n;c++)d||f.inArray(h[l][c],g)!=-1||(d=!0),k.push(h[l][c]);a=k.join("&");b=e[0];a&&(b=b+"?"+a);this.options.ajaxURL=b;return d},_parseDate:function(a){return d3.time.format("%Y-%m-%dT%H:%M:%S+00:00").parse(a)},_bindEvents:function(){var a=this,b="#"+this.widget_id;f(b+"-options legend").on("click",function(){f(this).siblings().toggle();f(this).children().toggle()});f(b+"-filters legend").on("click",
