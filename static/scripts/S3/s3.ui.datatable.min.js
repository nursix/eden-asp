...
*/
(function(e,w){function z(){this.data=[];this.slices=[];this.availableRecords=-1}var D=0,B=function(a,b){for(var c=0,d=b.length;c<d;c++)if(a==b[c])return c;return-1},E=function(a,b,c){a=a.split("?");b&&(a[0]+="."+b);c&&(a.length>1?a[1]+="&"+c:a.push(c));return a.join("?")},G=function(a,b){var c=function(g){return g.indexOf(".")!=-1||g[0]=="("},d=function(g,k){return g&&g.split("&").filter(function(h){h=h.split("=");return h.length>1&&k(S3.urlDecode(h[0]))})||[]};a=a.split("?");b=b.split("?");var f=
d(a[1],function(g){return!c(g)&&g[0]!="w"});a[1]=f.concat(d(b[1],c)).join("&");return a.join("?")};z.prototype.store=function(a,b,c){c!==w&&(this.availableRecords=c);if(c=b.length){var d=this.data,f=this.slices;b.forEach(function(k,h){d[a+h]=k});f.push([a,a+c]);f.sort(function(k,h){var m=k[0]-h[0];return m!==0?m:k[1]-h[1]});if(f.length>1){var g=[];b=f.reduce(function(k,h){return k[1]<h[0]||k[0]>h[1]?(g.push(k),h):[Math.min(k[0],h[0]),Math.max(k[1],h[1])]});g.push(b);this.slices=g}}};z.prototype.retrieve=
function(a,b){var c=this.availableRecords;if(c<0)return null;a<0&&(a=0);if(a>=c)return[];b=a+b;b>c&&(b=c);c=this.slices;for(var d,f=c.length,g=0;g<f;g++)if(d=c[g],a>=d[0]&&b<=d[1])return this.data.slice(a,b);return null};z.prototype.clear=function(){this.cache=[];this.slices=[];this.availableRecords=-1};e.widget("s3.dataTableS3",{options:{destroy:!1},_create:function(){this.id=D;D+=1;this.eventNamespace=".dataTableS3"},_init:function(){const a=e(this.element),b=a.attr("id");this.tableID=b;this.selector=
...
b=JSON.parse(b.val());b.rowActions.length?b.rowActionsJSON=!0:(b.rowActionsJSON=!1,b.rowActions=S3.dataTables.Actions?S3.dataTables.Actions:[]);var c=[];a=e("thead tr",a).children().length;for(var d=0;d<a;d++)c[d]=null;b.rowActions.length>0&&(c[b.actionCol]={sTitle:" ",bSortable:!1,className:"dt-actions actions"});b.bulkActions&&(a=" ",b.bulkSingle||(a='<div class="bulk-select-options"><input class="bulk-select-all" type="checkbox" title="'+i18n.selectAll+'"></input></div>'),c[b.bulkCol]={sTitle:a,
bSortable:!1,className:"dt-bulk"});if(b.colWidths){var f;a=b.colWidths;for(f in a)c[f]!=null?c[f].sWidth=a[f]:c[f]={sWidth:a[f]}}this.columnConfigs=c;return b}},_pipeline:function(a){var b=e.extend({cache:{},pages:2,data:null,method:"GET"},a);a=b.cache;var c=a.cacheLastRequest||null,d=a.cacheLastJson||null,f=a.cacheLower;f===w&&(f=-1);var g=new z;d&&f!=-1&&g.store(f,d.data,d.recordsFiltered||d.recordsTotal);var k=this;return function(h,m,p){if(this.hasOwnProperty("nTable")){if(h=p.sAjaxSource)k.ajaxUrl=
h,p.sAjaxSource=null;c=d=null;f=-1;g.clear();m({})}else{var n=!1,l=h.start,r=h.start,t=h.length,q=h.recordsTotal,u=q;d&&(d.recordsTotal!==w&&(q=d.recordsTotal),u=d.recordsFiltered!==w?d.recordsFiltered:q);k.totalRecords=q;t==-1&&(l=0,u!==w?t=u:n=!0);if(!n)if(q=l+t,p.clearCache)g.clear(),p.clearCache=!1,n=!0;else if(!c||JSON.stringify(h.order)===JSON.stringify(c.order)&&JSON.stringify(h.columns)===JSON.stringify(c.columns)&&JSON.stringify(h.search)===JSON.stringify(c.search)){var v=g.retrieve(l,q-
l);v===null&&(n=!0)}else g.clear(),n=!0;c=e.extend(!0,{},h);if(n){l<f&&(l-=t*(b.pages-1),l<0&&(l=0));f=l;h.start=l;h.length=t*b.pages;e.isFunction(b.data)?(v=b.data(h))&&e.extend(h,v):e.isPlainObject(b.data)&&e.extend(h,b.data);v=[{name:"draw",value:h.draw},{name:"limit",value:t==-1?"none":h.length}];l!=0&&v.push({name:"start",value:l});h.search&&h.search.value&&(v.push({name:"sSearch",value:h.search.value}),v.push({name:"iColumns",value:h.columns.length}));if(n=h.order.length){v.push({name:"iSortingCols",
...
(C.append("$search","session"),y.search=C.toString(),window.history.replaceState(null,null,y.href))}y=k.totalRecords;x.recordsFiltered!==w&&(y=x.recordsFiltered,k.totalRecords=x.recordsFiltered);g.store(l,x.data,y);d=e.extend(!0,{},x);l!=r&&x.data.splice(0,r-l);t!=-1&&x.data.splice(t,x.data.length);m(x)}})}else p=e.extend(!0,{},d,{draw:h.draw}),p.data=v,m(p)}}},_initCache:function(){var a=e(this.selector+"_dataTable_cache");return this.pipelineCache=a=a.length>0?JSON.parse(a.val()):{}},_headerCallback:function(){},
_rowCallback:function(){var a=this,b=this.tableConfig,c=b.actionCol,d=b.rowActions;return function(f,g){var k=/>(.*)</i.exec(g[c]);k=k===null?g[c]:k[1];if(d.length||b.bulkActions){for(var h=[],m=0;m<d.length;m++)h.push(a._renderActionButton(k,d[m]));e("td:eq("+c+")",f).html(h.join(""))}b.bulkActions&&a._bulkSelect(f,B(k,a.selectedRows));if(m=b.rowStyles){h=e(f);for(var p in m)B(k,m[p])!=-1&&h.addClass(p)}a._truncateCellContents(f,g);return f}},_drawCallback:function(){var a=this;return function(b){var c=
e(a.element),d=a.selector,f=a.outerForm,g=a.ajaxUrl;g&&f.find("a.permalink").each(function(){var l=e(this);l.attr("href",G(l.attr("href"),g))});a._renderBulkActions();a._variableColumnsButton();var k=b.fnRecordsDisplay();Math.ceil(k/b._iDisplayLength)>1?e(d+"_paginate").show():e(d+"_paginate").hide();f=e(".dt-export-options",f);f=e(".list_formats, .separator",f);k===0?f.hide():f.show();e(d+" .s3_modal").length&&S3.addModals();d=c.closest(".dt-contents");d.length&&(k>0?d.find(".empty").hide().siblings(".dt-wrapper").show():
...
b){var c="",d=b.restrict;if(d&&d.constructor===Array&&d.indexOf(a)==-1||(d=b.exclude)&&d.constructor===Array&&d.indexOf(a)!=-1)return c;c=b.label;b.icon?c='<i class="'+b.icon+'" alt="'+c+'"> </i>':b.img&&(c='<img src="'+b.icon+'" alt="'+c+'"></img>');d=b._disabled?' disabled="disabled"':"";const f=b._title||b.label,g=b._class;var k=/%5Bid%5D/g;b._onclick?(a=b._onclick.replace(k,a),c='<a class="'+g+'" onclick="'+a+d+'">'+c+"</a>"):b.url?(k=b.url.replace(k,a),(b=b._target||"")&&(b=' target="'+b+'"'),
...
p._bulkActionDialog(a,n.dialog,g):(e(".bulk-select-all",m).prop("checked",!1),p.selectedRows=[],p.selectionMode="Inclusive",m.dataTable().api().draw(!1),m.dataTable().fnReloadAjax());b.resolve()}})}else b.resolve();return b},_bulkActionDialog:function(a,b,c){let d=this.eventNamespace,f;try{f=e(b)}catch(p){return}let g=f;g.prop("tagName")!="FORM"&&(g=e("form",f));if(g.length){b=function(r,t,q=!1,u=!1){if(u||!e('input[name="'+r+'"]',g).length)u=document.createElement("input"),u.type="hidden",u.name=
...
for(r=1;r<c;r++)e('<span class="group-indent">').appendTo(f);c>1&&(e('<span class="ui-icon ui-icon-triangle-1-e group-closed">').appendTo(f),e('<span class="ui-icon ui-icon-triangle-1-s group-opened">').hide().appendTo(f));r="";k[b]!=null?r=" ("+k[b]+")":(h+=b,k[h]!=null&&(r=" ("+k[h]+")"));f.append(b+r);l&&m&&(b=n.groupIcon|0,b=b.length>=c?b[c-1]:"icon",k=e('<span class="group-expand">').appendTo(f),m=e('<span class="group-collapse">').hide().appendTo(f),b=="text"?(k.text("\u2192"),m.text("\u2193")):
b=="icon"&&(k.addClass("ui-icon ui-icon-arrowthick-1-e"),m.addClass("ui-icon ui-icon-arrowthick-1-s")));p?d.insertAfter(a):d.insertBefore(a);n.groupSpacing&&(a=d.prevAll("tr.group").first(),a.length&&a.data("level")==c&&(a=a.data("group"),g=e("<td>").attr("colspan",g),g=e('<tr class="spacer">').append(g),l&&g.addClass("collapsable"),g.addClass("xgroup_"+c+"_"+a).insertBefore(d)))},_toggleGroup:function(a,b){switch(this.tableConfig.shrinkGroupedRows){case "individual":b?this._expandGroup(a):this._collapseGroup(a);
...
k+"&iColumns="+c.length);m!==null&&(h=m.concat(h));c.forEach(function(p,n){p.bSortable||(g+="&bSortable_"+n+"=false")});g+="&iSortingCols="+h.length;h.forEach(function(p,n){g+="&iSortCol_"+n+"="+h[n][0]+"&sSortDir_"+n+"="+h[n][1]});d=E(d,f,g)}else d=E(d,f);e.searchDownloadS3!==w?e.searchDownloadS3(d,"_blank"):window.open(d)}},_initExportFormats:function(){var a=this._parseConfig();if(a!==w&&(a=a.ajaxUrl)&&S3.search!==w){var b=document.createElement("a");b.href=a;if(b.search){var c=b.search.slice(1).split("&").map(function(d){d=
d.split("=");return[S3.urlDecode(d[0]),S3.urlDecode(d[1])]}).filter(function(d){return d[0].indexOf(".")!=-1});e(".dt-export",this.outerForm).each(function(){var d=e(this),f=d.data("url");f&&d.data("url",S3.search.filterURL(f,c))})}}},_bindEvents:function(){var a=e(this.element),b=this.outerForm,c=this.eventNamespace,d=this;a.on("click"+c,".dt-truncate .ui-icon-zoomin, .dt-truncate .ui-icon-zoomout",function(){e(this).parent().toggle().siblings(".dt-truncate").toggle();return!1});this._initExportFormats();
...
function(a,b){b!="undefined"&&b!=null&&(a.sAjaxSource=b);this.oApi._fnProcessingDisplay(a,!0);var c=this;a.ajax({},function(){c.oApi._fnClearTable(a);c.fnDraw()},a)};e(function(){if(S3.dataTables){var a=S3.dataTables.id;a&&a.forEach(function(b){e("#"+b).dataTableS3({destroy:!1})})}})})(jQuery);
