...
e,type:"DELETE",dataType:"json",retryLimit:0,success:function(h){d.val("");f.remove();delete l.columnConfigs[e]},always:function(){k.hide()}})}}},_toggleConfigSave:function(c,b){const d=a(".cfg-select",c),f=a(".cfg-save",c),e=a(".cfg-select-options",c);c=a("input.cfg-save-name",c);if(b){b=a("option:selected",e);let g="",k=0;e.val()&&(g=b.text().trim(),k=g.length*2);d.hide();f.removeClass("hide").show();c.val(g).focus();c[0].setSelectionRange(k,k)}else c.val(""),f.hide(),d.removeClass("hide").show()},
