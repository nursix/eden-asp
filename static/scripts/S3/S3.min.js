...
m;var p=function(t){t=$(t);if(t.hasClass("responsive"))(t=t.DataTable())&&t.responsive&&t.responsive.recalc();else if(t.hasClass("doublescroll"))try{t.dataTableS3("doubleScroll")}catch(u){}},r=function(t,u){var y=u.find(".map_wrapper"),v=y.length;v?$.when(n()).then(function(){for(var C=S3.gis,A=0;A<v;A++){var z=y[A].attributes.id.value;void 0===C.maps[z]&&C.show_map(z,C.options[z])}l(t)},function(C){s3_debug(C)},function(C){s3_debug(C)}):l(t);u.find("table.dataTable.display").each(function(){p(this)})};
S3.search.unhide_section=r;S3.search.summary_tabs=function(t,u,y){if(y)for(var v=f($("#"+t)),C=y.split(","),A=0,z=C.length;A<z;A++)y=C[A],m(t,y,v);void 0!=u?($("#summary-tabs").tabs({active:u,activate:function(D,F){D=$(F.newPanel);D.removeClass("hide");if(F.newTab.length){var E=$(F.newTab).index();F=$("#"+t).find('input.filter-submit-url[type="hidden"]');if(F.length){F=F.first();var G=$(F).val(),I=G.split("?"),H=[];if(1<I.length){G=I[1].split("&");for(var J,M,R=0;R<G.length;R++)J=G[R].split("="),
...
y=0;y<u.length;y++){var v=u[y];if(!v.hidden){var C=S3.gis;v=v.attributes.id.value;if(void 0===C.maps[v]){var A=C.options[v];void 0!=A&&(C.show_map(v,A),v=f($("#"+t)),C.refreshLayer("search_results",v))}}}};S3.search.s3map=function(t){var u=S3.gis;void 0===t&&(t="default_map");u.show_map(t,u.options[t]);t=f();u.refreshLayer("search_results",t)};var w=function(t){var u=t.name,y=u.slice(0,-1),v=parseInt(u.slice(-1)),C=$("#"+u);if(C.val()){var A=v+1,z=y+A,D=$("#"+z);D.length||(A=v+2,z=y+A,D=$("#"+z));
if(D.length){var F=z.replace(/-/g,"_");S3[F]();D.next(".ui-multiselect").show();F=S3.location_filter_hierarchy;if(void 0!=S3.location_name_l10n){D=!0;var E=S3.location_name_l10n}else D=!1;t.options1=[];t.options2=[];t.options3=[];t.options4=[];t.options5=[];for(u=v;0<=u&&!F.hasOwnProperty("L"+u);u--);var G=function(L){var O,P;for(O in L)if(L.hasOwnProperty(O)){O&&"null"!=O&&t["options"+A].push(O);var Q=L[O];if("object"===typeof Q&&(z=A+1,$("#"+y+z).length||(z=A+2),$("#"+y+z).length))for(P in Q)Q.hasOwnProperty(P)&&
P&&"null"!=P&&t["options"+z].push(P)}},I=function(L,O){var P,Q,U=$("#"+y+O).val();for(Q in L)if(L.hasOwnProperty(Q))if(null===U)if(O===v)G(L[Q]);else{var N=O+1;$("#"+y+N).length||N++;I(L[Q],N)}else for(P in U)U[P]===Q&&(O===v?G(L[Q]):(N=O+1,$("#"+y+N).length||N++,I(L[Q],N)))};I(F["L"+u],u);for(F=A;5>=F;F++){var H=$("#"+y+F);if(H.length){var J=t["options"+F];J.sort();var M="";for(var R in J)if(J.hasOwnProperty(R)){u=J[R];var S=D?E[u]||u:u;M+='<option value="'+u+'">'+S+"</option>"}H.html(M);if(H.hasClass("groupedopts-filter-widget")&&
...
function(){$(this).closest("form").trigger("optionChanged")});$(".hierarchy-filter").on("select.s3hierarchy",function(){$(this).closest("form").trigger("optionChanged")});$(".map-filter").each(function(){var t=$(this);$.when(n()).then(function(){var u=t.attr("id"),y=$("#"+u),v,C=S3.gis,A=C.maps[u+"-map"],z=A.s3;z.pointPlaced=function(D){var F={internalProjection:A.getProjectionObject(),externalProjection:C.proj4326};v=(new OpenLayers.Format.WKT(F)).write(D);y.val('"'+v+'"').trigger("change")};z.polygonButtonOff=
function(){y.val("").trigger("change")};z.layerRefreshed=function(){var D=z.polygonButton;D&&"drawpolygonclear-off"==D.getIconClass()&&(z.lastDraftFeature?z.lastDraftFeature.destroy():1<z.draftLayer.features.length&&z.draftLayer.features[0].destroy(),D.control.deactivate(),D.items[0].pressed=!0)};z.polygonButtonLoaded=function(){if(v=y.val()){z.polygonButton.items[0].btnEl.dom.click();var D=OpenLayers.Geometry.fromWKT(v);D.transform(C.proj4326,A.getProjectionObject());D=new OpenLayers.Feature.Vector(D,
