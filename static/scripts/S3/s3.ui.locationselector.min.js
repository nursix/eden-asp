...
n.multiselect("refresh")):a=parseInt(n.val(),10);if(b===0){var t=f._readLabels(a),u=q.d,r=["1","2","3","4","5"],v,w,y;t.then(function(K){for(y=0;y<5;y++)m=r[y],v=K[m]||u[m],x=g+"_L"+m,n=d(x),w=l.showLabels?n.hasClass("required")?"<div>"+v+':<span class="req"> *</span></div>':v+":":"",d(x+"__row label").html(w),d(x+"__row1 label").html(w),d(x+' option[value=""]').html(i18n.select+" "+v),n.hasClass("multiselect")&&n.multiselect("instance")&&n.multiselect("refresh")})}t=l.hideLx;for(m=b+1;m<6;m++){var x=
g+"_L"+m;t?(d(x+"__row").hide(),d(x+"__row1").hide()):d(x+" option").remove('[value != ""]');d(x).val("").trigger("change","implicit")}if(a){var z=!1,A=b+1,E=d(g+"_L"+A+"__row");E.length||(z=!0,A++,E=d(g+"_L"+A+"__row"));if(E.length){var B;t=!1;if(d(g+"_L"+b+' option[value="'+a+'"]').hasClass("missing")){var C=k[a];C.i=a;var F=[C]}else for(B in F=[],t=!0,k)if(k[B].f==a){t=!1;break}f._readHierarchy(t,a,A,z).then(function(){if(F.length==0)for(B in k)C=k[B],C.f==a&&(C.i=B,F.push(C));var K=F.length;if(K){F.sort(function(L,
...
(D.multiselect(H).multiselectfilter({label:"",placeholder:i18n.search}),d(".ui-multiselect-header").show()):(H.header=!1,D.multiselect(H)));D=f.data["L"+A];H=F.map(function(L){return L.i});D&&H.indexOf(""+D)!=-1?f.lxSelect(A,D,c):K==1&&B&&f.lxSelect(A,B,c)}f._lxSelectFinal(c);h.resolve()})}else f.useGeocoder&&!c&&f._geocodeDecision(),f._lxSelectFinal(c),h.resolve()}else f._lxSelectFinal(c),h.resolve();return h.promise()},_collectData:function(b){var a=this.data,c="#"+this.fieldname,e=this._lookupParent(),
h=d(c+"_address").val(),f=d(c+"_postcode").val();a.address=h;a.postcode=f;if(a.specific)a.id=a.specific,a.parent=e;else{var g=a.lat,l=a.lon,n=a.wkt;h||f||g||l||n?(a.id=null,a.parent=e):(a.id=e,a.parent=null)}this._serialize();d(c+"_lat").val(a.lat).trigger("setvalue");d(c+"_lon").val(a.lon).trigger("setvalue");this.input.data("input",this._hasData());this.fieldname.slice(0,4)=="sub_"&&this.input.trigger("change",b&&"user"||"implicit")},_collectLx:function(){var b=this.data,a="#"+this.fieldname,c;
for(c=0;c<6;c++){var e=d(a+"_L"+c);e.length&&(e=e.val(),b["L"+c]=e?parseInt(e,10):null)}this._serialize()},_hasData:function(){var b=this.data,a=!1;b.specific||b.address||b.postcode||b.lat||b.lon||b.wkt?a=!0:[b.L0,b.L1,b.L2,b.L3,b.L4,b.L5].join("|")!=this.lx&&(a=!0);return a},_lookupParent:function(){this._collectLx();for(var b=this.data,a,c=5;c>-1;c--)if(a=b["L"+c])return a;return(b=k.d)?b.i:null},_readLabels:function(b){b||="d";var a=d.Deferred(),c=q[b];if(c==I){var e=S3.Ap.concat("/gis/hdata/"+
b);d.ajaxS3({url:e,dataType:"json",success:function(h){c={};try{for(var f in h)c[f]=h[f];q[b]=c}catch(g){}a.resolve(c)},error:function(h,f,g){(h=g=="UNAUTHORIZED"?i18n.gis_requires_login:h.responseText)&&s3_debug(h);a.reject()}})}else a.resolve(c);return a.promise()},_readHierarchy:function(b,a,c,e){var h=""+a+";"+c+";"+e,f=p[h];if(f!==I)return f;var g=d.Deferred();if(b){b="#"+this.fieldname;var l=d(b+"_L"+c),n=!1;l.hide();if(l.hasClass("multiselect")){n=!0;var m=d(b+"_L"+c+"__row button").hide()}var t=
d(b+"_L"+c+"__throbber").removeClass("hide").show();a=e?S3.Ap.concat("/gis/ldata/"+a+"/"+c):S3.Ap.concat("/gis/ldata/"+a);d.ajaxS3({url:a,dataType:"json",success:function(u){for(var r in u)k[r]=u[r];t.hide();n?m.removeClass("hide").show():l.removeClass("hide").show();g.resolve()},error:function(u,r,v){if(u=v=="UNAUTHORIZED"?i18n.gis_requires_login:u.responseText)s3_debug(u),S3.showAlert(u,"error");t.hide();n?m.removeClass("hide").show():l.removeClass("hide").show();g.reject()}})}else g.resolve();
return f=p[h]=g.promise()},_geocodeDecision:function(){var b="#"+this.fieldname,a=this.data;this._collectData();if(S3.gis.geocodeDecisionPlugin)S3.gis.geocodeDecisionPlugin(b,a,this);else if(a.address){a=["1","2","3","4","5"];for(var c=0,e;c<5;c++)if(e=d(b+"_L"+a[c]),e.length&&!e.val()&&e[0].options.length>1)return;d(b+"_geocode .geocode_success,"+b+"_geocode .geocode_fail").hide();var h=this;a=this.eventNamespace;if(this.input.data("manually_geocoded"))d(b+"_geocode button").removeClass("hide").show().off(a).on("click"+
a,function(){d(this).hide();h._geocode()});else this._geocode()}},_geocode:function(){for(var b=this.fieldname,a=this,c="#"+b,e=d(c+"_geocode .geocode_fail").hide(),h=d(c+"_geocode .geocode_success").hide(),f=d(c+"_geocode .throbber").removeClass("hide").show(),g=this.data,l={address:g.address},n="postcode L0 L1 L2 L3 L4 L5".split(" "),m=0,t,u;m<7;m++)t=n[m],(u=g[t])&&(l[t]=u);l.k=d(c+"_geocode button").data("k");c=S3.Ap.concat("/gis/geocode");d.ajaxS3({url:c,type:"POST",data:l,dataType:"json",success:function(r){var v=
...
for(e=0;e<6;e++){var g=b[e];var l=a+"_"+g;var n=d(l);if(n.length&&n.hasClass("required")&&h(n)&&!c[g]){this._fieldError(n,i18n.enter_value);f=!0;break}}["address","postcode"].forEach(function(m){l=a+"_"+m;n=d(l);n.length&&n.hasClass("required")&&h(n)&&!c[m]&&(this._fieldError(n,i18n.enter_value),f=!0)},this);return!f},_serialize:function(){var b=JSON.stringify(this.data);this.input.val(b);return b},_deserialize:function(){var b=this.input.val();return this.data=JSON.parse(b)},_storeHierarchyData:function(b,
a){var c;if(a)for(c in a)k[c]=a[c];if(b)for(c in b)q[c]=b[c]},_jsLoaded:function(){var b=new jQuery.Deferred;setTimeout(function c(){S3.gis.maps!=I?b.resolve("loaded"):b.state()==="pending"&&(b.notify("waiting for JS to load..."),setTimeout(c,500))},1);return b.promise()},_fieldError:function(b,a){b.addClass("invalidinput").after('<div class="error" style="display: block;">'+a+"</div>");d(".invalidinput").get(0).scrollIntoView()},_removeErrors:function(b){b||(b="#"+this.fieldname,b=b+"_L0,"+b+"_L1,"+
...
d(e+"_address__row1").removeClass("hide").show();d(e+"_L0__row").removeClass("hide").show();d(e+"_L0__row1").removeClass("hide").show();var f=c.data,g=f.L2,l=f.L3,n=f.L4,m=f.L5,t;[f.L0,f.L1||g,g||l,l||n,n||m,m].forEach(function(u,r){u&&(t=t?t.then(function(){return c.lxSelect(r,u,!0)}):c.lxSelect(r,u,!0))});d(this).hide()});d(e+"_lat,"+e+"_lon").on("change"+a,function(){c._latlonInput()});d(e+"_latlon_toggle").on("click"+a,function(){var f=d(this).data("mode")||c.options.latlonMode;if(f=="dms"){f=
"decimal";var g=i18n.latlon_mode.dms}else f="dms",g=i18n.latlon_mode.decimal;d(e+"_lat").latloninput("option",{mode:f});d(e+"_lon").latloninput("option",{mode:f});d(this).html(g).data("mode",f)});if(b.substring(0,4)=="sub_")this.input.closest(".inline-form").on("validate"+a+this.id,function(f){c._validate()||f.preventDefault()});else{var h=this.input.closest("form");h.on("submit"+a+this.id,function(f){f.preventDefault();c._validate()&&h.off(a+c.id).trigger("submit")})}return!0},_unbindEvents:function(){var b=
...
h;this.secInput=g;this.dmsInput=c=d("<span>").append(c).append(e).append(h).append(f).append(g).append(a).hide();this.input=p.append(b).append(c).show()}this._removeErrors();q.mode=="dms"?(k=this._getDMS(),this.degInput.val(k.d),this.minInput.val(k.m),this.secInput.val(k.s),this.decimalInput.hide(),this.dmsInput.show()):(this.decimalInput.val(k.val()),this.dmsInput.hide(),this.decimalInput.show());this._bindEvents()},_getDMS:function(){var k=d(this.element).val();if(isNaN(parseFloat(k))||!isFinite(k))return{d:"",
m:"",s:""};var q=Math.abs(k);q=(q-parseInt(q,10))*60;if(Math.abs(q-Math.round(q))<1E-10){q=Math.round(q);var p=0}else p=(q-parseInt(q,10))*60,Math.abs(p-Math.round(p))<1E-10&&(p=Math.round(p));return{d:parseInt(k,10),m:parseInt(q,10),s:p}},_showError:function(k,q){k=="all"?this.input.find("input").addClass("invalidinput"):k&&k.addClass("invalidinput");q&&this.input.append('<div class="error">'+q+"</div>")},_removeErrors:function(){this.input.find("input").removeClass("invalidinput");this.input.find(".error").remove()},
_validateDMS:function(){this._removeErrors();if(this.options.type=="lat"){var k=90;var q=i18n.latlon_error.lat}else k=180,q=i18n.latlon_error.lon;var p=this.degInput.val(),b=this.minInput.val(),a=this.secInput.val(),c=[];if(p===""&&b===""&&a==="")return"";p=parseInt(p,10)||0;b=parseInt(b,10)||0;a=parseFloat(a)||0;Math.abs(b)>=60&&(this._showError(this.minInput),c.push(i18n.latlon_error.min));Math.abs(a)>=60&&(this._showError(this.secInput),c.push(i18n.latlon_error.sec));b=Math.abs(p)+b/60+a/3600;
if(!c.length&&b>k||p>k)this._showError("all"),c.push(q);return c.length?(this._showError(null,c.join(", ")),null):(p<0?-1:1)*b},_validateDecimal:function(){this._removeErrors();var k=this.options.type,q=i18n.latlon_error.format;if(k=="lat"){var p=90;var b=i18n.latlon_error.lat}else p=180,b=i18n.latlon_error.lon;var a=this.decimalInput.val();if(a==="")return"";a=this._parse(a,k);a===null?this._showError(this.decimalInput,q):Math.abs(a)>p&&(this._showError(this.decimalInput,b),a=null);return a},_parse:function(k,
q){var p=q=="lat"?{N:1,S:-1}:{E:1,W:-1};var b=/^([NSEW]{0,1})\s*([-+]?\d+[.,]?\d*)\s*\u00b0?\s*([NSEW])?\s*$/,a=/^([NSEW]{0,1})\s*([-+]?\d+[.,]?\d*)?\s*([\u00b0'"]{0,1})\s*:?\s*([-+]?\d+[.,]?\d*)?\s*(['"]{0,1})\s*:?\s*([-+]?\d+[.,]?\d*)?\s*(['"]{0,1})\s*([NSEW])?\s*$/,c=q=0,e=0;if(k.match(b))return a=k.replace(b,"$1|$2|$3").split("|"),(k=a[0]||a[2])&&(k=p[k]),q=parseFloat(a[1]),k&&Math.abs(q)!=q&&(k=null),k&&(q*=k),q;if(k.match(a)){a=k.replace(a,"$1|$2|$3|$4|$5|$6|$7|$8").split("|");(k=a[0]||a[7])&&
(k=p[k]);p=[];b=[];var h,f;for(f=1;f<6;f+=2){var g=a[f];(h=a[f+1])?(p.push(parseFloat(g)||0),b.push(h)):g&&(p.push(parseFloat(g)),b.push(null))}if(a=p.length){for(f=1;f<a;f++)p[f]=Math.abs(p[f]);g=p[0];Math.abs(g)!=g&&(k=1);if(a==1)h=b[0],h=='"'?e=p[0]:h=="'"?c=p[0]:h&&h!="\u00b0"||(q=p[0]);else if(a==2)if(a=b[0],b=b[1],a=="\u00b0")q=p[0],b=='"'?e=p[1]:b!="'"&&b||(c=p[1]);else if(a){if(a!="'"||b&&b!='"')return null;c=p[0];e=p[1]}else b=='"'?(c=p[0],e=p[1]):b!="'"&&b||(q=p[0],c=p[1]);else{if(b[0]&&
b[0]!="\u00b0"||b[1]&&b[1]!="'"||b[2]&&b[2]!='"')return null;q=p[0];c=p[1];e=p[2]}q=q+c/60+e/3600;k&&(q*=k);return q}}return null},_bindEvents:function(){var k=this,q=this.eventNamespace;this.dmsInput.find("input").on("change"+q,function(){var p=k._validateDMS();p!==null?d(k.element).val(p).trigger("change"):d(k.element).val(k.defaultValue).trigger("change")});this.decimalInput.on("change"+q,function(){var p=k._validateDecimal();p!==null?(d(k.element).val(p).trigger("change"),k.decimalInput.val(p)):
