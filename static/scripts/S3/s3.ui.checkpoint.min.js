...
m=f.data("member"),n=m.r,p=!a,t="";if(n&&n.hasOwnProperty(l)){n=n[l];var u=n[1]?new Date(n[1]):null;if(u===null||u>h)p=!0,t=b('<div class="member-message">'+n[0]+"</div>")}f.find(".member-message").remove();p?(f.removeClass("member-selected"),b(".member-select",f).each(function(){b(this).prop("checked",!1).prop("disabled",!0)}),f.addClass("member-blocked"),p=b('<div class="member-message row">'),b('<div class="columns">').append(t).appendTo(p),f.find(".member-info.row").after(p)):(g++,f.removeClass("member-blocked"),
...
this.eventTypeSelect).filter(function(){return b(this).data("code")==a});return d.length?{code:d.data("code")||null,name:d.data("name")||null,multiple:d.data("multiple")=="T"}:null},_setEventType:function(a,d){b('input[type="hidden"][name="event"]').val(a);b(".event-type-name",this.eventTypeHeader).text(d);this._updateEventType();this._updateFamilyStatus();b(this.idPrefix+"_person__row .controls").text()&&this._toggleSubmit(!0)},_clearEventType:function(){b('input[type="hidden"][name="event"]').val("");
this._updateEventType();this._toggleSubmit(!1)},_updateEventType:function(){let a=this.eventTypeHeader,d=b("a.event-type-select",this.eventTypeSelect).length,e=b('input[type="hidden"][name="event"]').val();var c=this.options;e?a.removeClass("challenge"):(c=d>0?c.selectEventLabel:c.noEventsLabel,b(".event-type-name",a).text(c));d==0?a.addClass("empty").addClass("disabled"):d==1&&e?a.removeClass("empty").addClass("disabled"):(a.removeClass("empty").removeClass("disabled"),e||a.addClass("challenge"))},
_populateEventTypes:function(a){let d=this.eventTypeSelect.empty(),e=a.types;(a=a.default)||e.length!=1||(a=e[0]);e.forEach(function(c){let g=c[0],l=c[1];c=c[2]?"T":"F";b('<a class="secondary button event-type-select">').text(l).appendTo(d).data({code:g,name:l,multiple:c})});a?this._setEventType(a[0],a[1]):this._clearEventType()},_selectOrg:function(a){let d=a.data("id");a=a.data("name");let e=this.orgHeader,c=this.orgSelect;this._clearForm();e.data("selected",d).addClass("selected");b("h4.org-name",
e).text(a);let g=b('<div class="inline-throbber">'),l=this.eventTypeHeader.addClass("disabled"),k=b(".event-type-name",l).hide().after(g),h=this;b.ajaxS3({url:this.options.ajaxURL+"?org="+d,type:"GET",dataType:"json",contentType:"application/json; charset=utf-8",success:function(f){h._populateEventTypes(f);g.remove();k.show();f.types.length>1&&l.removeClass("disabled")},error:function(){g.remove()}});c.slideUp("fast")},_selectEventType:function(a){let d=a.data("code");a=a.data("name");this._setEventType(d,
